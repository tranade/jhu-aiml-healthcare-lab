  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="lg:hidden fixed top-16 left-0 right-0 z-50 bg-gray-950/95 backdrop-blur-md border-b border-gray-800/50 overflow-hidden"
      >
        <div className="container-max mobile-padding">
          <div className="py-4 space-y-2">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setIsOpen(false)}
                className="block px-4 py-3 text-gray-300 hover:text-white hover:bg-gray-800/50 rounded-lg transition-colors font-medium"
              >
                {item.name}
              </Link>
            ))}
